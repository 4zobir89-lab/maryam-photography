  ...
  return (
    <section id="portfolio" className="py-32 md:py-48 bg-background">
      <div className="max-w-6xl mx-auto px-6">
        {/* Minimal header — title + filter in one row */}
        ...
        {/* Grid */}
        {filtered.length > 0 ? (
          <motion.div layout className="columns-1 sm:columns-2 lg:columns-3 gap-4 md:gap-5 [column-fill:_balance]">
            <AnimatePresence mode="popLayout">
              {filtered.map((project, i) => (
                <motion.button
                  ...
                  className="group relative w-full block mb-4 md:mb-5 break-inside-avoid overflow-hidden bg-card text-right"
                >
                  {project.imageData ? (
                  ...
                  <div className="absolute bottom-0 inset-x-0 p-5 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-2 group-hover:translate-y-0">
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="eyebrow">{project.year}</span>
                      <span className="font-inter text-[9px] tracking-[0.2em] text-muted-foreground uppercase">
                        {categories.find((c) => c.id === project.category)?.labelAr}
                      </span>
                    </div>
                    <h3 className="font-amiri text-lg text-foreground">{project.titleAr}</h3>
                    <p className="font-display text-[11px] text-muted-foreground mt-0.5">{project.location}</p>
                  </div>
                </motion.button>
              ))}
            </AnimatePresence>
      ...
      <AnimatePresence>
        {selected && (
          <motion.div
            ...
            className="fixed inset-0 z-[80] bg-background/95 flex flex-col"
          >
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  {galleryImages[currentIdx] && (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={galleryImages[currentIdx]} alt={selected.titleAr} className="w-full max-h-[70vh] object-contain" />
                  )}
                </motion.div>
              </AnimatePresence>
              {galleryImages.length > 1 && (
